...
Regression.prototype._indexData = function(){
  ...
  var currentProcessedData = db.indexJobData(currentJobData);
  this.skippedInitialCount = currentProcessedData.skipped;

  var data = {
    current: currentProcessedData.indexed,
    prev: db.indexJobData(prevJobData).indexed
  }
...
Regression.prototype.analyze = function(data){
  ...
  this.count = {
    skipped: this.skippedInitialCount,
    match: 0,
    noMatch: 0,
    total: Object.keys(data.current).length + this.skippedInitialCount,
    noMatchData: []
  }
